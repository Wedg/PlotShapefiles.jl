...
function plotshape(shparray::AbstractArray{Shapefile.ESRIShape, 1}, canvas::Compose.Context;
                   convertcoords=lonlat_to_webmercator, img_width=12cm,
                   line_width=0.05mm, line_color="black", fill_color=nothing, radius=[0.5cm])

    # Plot and return canvas
    draw_shp(concrete(shparray), canvas, convertcoords, line_width, line_color, fill_color, radius)

end

# Plot Shapefile - given array of shapes and an MBR
function plotshape(shparray::AbstractArray{Shapefile.ESRIShape, 1}, MBR::Shapefile.Rect{Float64};
...
function plotshape(shparray::AbstractArray{Shapefile.ESRIShape, 1}; options...)
