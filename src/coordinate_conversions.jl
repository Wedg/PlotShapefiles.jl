...
#=
Calculates parameters needed for the Google API to match the shapefile
...
function bbox_coords_and_zoom_to_center_and_size(cxEast, cyNorth, cxWest, cySouth, zoom)
    ...
    pxEast, pyNorth = WGS84toWebMerc(cxEast, cyNorth, zoom)
    pxWest, pySouth = WGS84toWebMerc(cxWest, cySouth, zoom)

    # Calculate image size in pixels
    width = pxEast - pxWest
    height = pySouth - pyNorth

    # Calculate center coordinates in Lon/Lat
    mid_cxLon = (cxEast + cxWest) / 2
    mid_cyLat = (cyNorth + cySouth) / 2

    # Return center x, center y, width and height
    mid_cxLon, mid_cyLat, ceil(Int, width), ceil(Int, height)
end

function bbox_coords_and_zoom_to_center_and_size(MBR::Shapefile.Rect{Float64}, zoom)
